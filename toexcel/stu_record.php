...
//$cla='1010101';
$sql="select stu_id,user_name,stu_sex,stu_nation,nature from user_stu where stu_clas='$cla'";
...
$worksheet->write(0, 0, "烟台大学留学生教学记录表".encoding($_SESSION['term_now']), $format_title);
...
$worksheet->write(1, 0, '班级：'.$class, $format_bold_border_three_l);
...
while($re=mysql_fetch_assoc($query)){
	...
	$re['nature']=encoding($re['nature']);
	$re['stu_nation']=encoding($re['stu_nation']);
...
$workbook->send('记录表_'.$class.'_'.encoding($_SESSION['term_now']).'.xls');
$workbook->close();
?>