...
$worksheet->write(0, 0, "烟台大学留学生教学记录表(".encoding($termnow).')', $format_title);
...
while($re=mysql_fetch_assoc($query)){
    ...
    $ii++;
    $worksheet->write($jj, 1, encoding($re['stu_id']),$format_text);
    ...
    $worksheet->write($jj, 5, encoding($re['stu_nation']),$format_cell_r_2);
    for($kk=6;$kk<=40;$kk++){
        if($kk%5==0){
