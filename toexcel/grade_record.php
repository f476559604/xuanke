...
else{$ntea=$_SESSION['name'];$ntea=encoding($ntea);}
if($lesson==''||$termnow==''){
    exit('操作失败');
}
/*$class=obj_name($cla);
...
$worksheet->write(0, 0, "烟台大学留学生成绩记录表".encoding($termnow), $format_title);
...
if($_SESSION='education'){
    ...
    $worksheet->write(1, 0, "课号：".$lesson." 课程名：".$ncou." 任课老师：".encoding($_SESSION['name']), $format_title_1);
}
...
while($re=mysql_fetch_assoc($query)){
    ...
    $worksheet->write($kk, 2, encoding($re['user_name']), $format_border);
    $worksheet->write($kk, 3, $class, $format_border);
    $worksheet->write($kk, 4, encoding($re['cou_stu_mark1']), $format_border);
    $worksheet->write($kk, 5, encoding($re['cou_stu_mark2']), $format_border);
    $worksheet->write($kk, 6, encoding($re['cou_stu_mark3']), $format_border);
    $worksheet->write($kk, 7, encoding($re['cou_stu_mark']), $format_border);
...
$worksheet->write($kk, 0, '考试时间：'.encoding($termnow), $format_border_left);
...
if($_SESSION['type']=='education'){
    $workbook->send('成绩表_'.$ncou.'_'.$ntea.'_'.encoding($termnow).'.xls');
}
else{
    $workbook->send('成绩表_'.$ncou.'_'.encoding($_SESSION['name']).'_'.encoding($termnow).'.xls');
}
unset($ntea);
$workbook->close();
?>