...
$cou_sql="select * from `room_course` where room_name='".$nroom."' and cou_term='".$term."' limit 1";
//echo $cou_sql;

$cou_sql=mysql_fetch_assoc(mysql_query($cou_sql));
$filenm=iconv("utf8","gb2312",$nroom)."教室课程表";
...
for($i=1;$i<=8;$i++){
	...
	$worksheet->write($i+1, 3, iconv("utf8","gb2312",$cou_sql['th'.$i]), $format_border);
	$worksheet->write($i+1, 4, iconv("utf8","gb2312",$cou_sql['we'.$i]), $format_border);
