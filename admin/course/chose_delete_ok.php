...
//print_r($del_cou);
$cou_sql="select cou_all from stu_course where stu_id='".$_SESSION['user_id']."' and cou_term='".$_SESSION['term_now']."'";
...
foreach($del_cou as $value){
    ...
    $del_sql="update stu_course set ".$cou_sql_up." ,cou_all='".$end_c."' where stu_id='".$_SESSION['user_id']."' and cou_term='".$_SESSION['term_now']."'";
    //print_r($del_class);
    //echo $cou_time_num;
...
if($i==1){
    echo('<script>alert("删除成功");window.location.href="chose_delete.php";</script>');
}
?>