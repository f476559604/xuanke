...
for($i=0;$i<count($name_name);$i++){
    ...
    $jud_cou_time="select ".$cou_info[1]." from maj_course where class_id='".$num."' and cou_term='".$_SESSION['term_now']."'";
    //echo $jud_cou_time;
    $jud_cou_re=mysql_query($jud_cou_time);
    $jud_cou_re=mysql_fetch_assoc($jud_cou_re);
    //print_r($jud_cou_re);
    ...
    //echo 'sdfs';
        ...
        $judge_sql="select class_id from maj_course where class_id='".$num."' and cou_term='".$_SESSION['term_now']."'";
        
        $judge_sql=mysql_fetch_row(mysql_query($judge_sql));
    
        if($judge_sql[0]!=NULL){
            $couse_all_sql="select cou_all from maj_course where class_id='".$num."' and cou_term='".$_SESSION['term_now']."'";
            //echo($couse_all_sql);
            $couse_all_re=mysql_query($couse_all_sql);
            $couse_all_re=mysql_fetch_row($couse_all_re);
            $couse_all=$couse_all_re[0].'#'.$name_name[$i].'#';
            $cou_sql="update maj_course set ".$cou_sql_up." ,cou_all='".$couse_all."' where class_id='".$num."' and cou_term='".$_SESSION['term_now']."'";
            //echo($cou_sql);
        }
        else{$cou_sql="insert into maj_course(class_id,".$cou_info[1].",cou_all,cou_term) values('".$num."',".$cou_time_sql.",'#".$name_name[$i]."#','".$_SESSION['term_now']."')";}
        //echo $cou_sql;
...
if($ii=='1'){
    echo('<script>alert("选课成功");window.history.back(-1);</script>');
}
//print_r($cou_sql);
