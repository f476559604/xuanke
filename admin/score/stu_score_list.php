...
function stu_score_list($termnow,$aid=0){
    global $db;
    //global $stu_id;
    ...
    //echo "ssdd";
    if($termnow=='all'){
        $sql="select * from cou_choose where cou_choose_stu='".$stu_id."'";
        $sql1="select cou_all_id,cou_name,cou_credit,cou_stu_mark,cou_term from before_cou where cou_choose_stu='".$stu_id."'";
        //echo $sql;
    }
    else if($termnow=='pass'){
        $sql="select * from cou_choose where cou_choose_stu='".$stu_id."' and cou_stu_mark >= 60";
        //echo $sql;
    }
    else if($termnow=='nopass'){
        $sql="select * from cou_choose where cou_choose_stu='".$stu_id."' and cou_stu_mark  < 60";
        //echo $sql;
    }
    else if($termnow=='search_id'){
        $sql="select * from cou_choose where cou_choose_stu='".$aid."' order by cou_term";
        //echo $sql;
    }
    else{$sql="select * from cou_choose where cou_choose_stu='".$stu_id."' and cou_term='".$termnow."'";}
    ...
    while($head_info1=mysql_fetch_assoc($query)){
        ...
        foreach($head_info1 as $key=>$value){
            if($key=='cou_all_id'||$key=='cou_choose_stu'||$key=='cou_edit_ok'){
